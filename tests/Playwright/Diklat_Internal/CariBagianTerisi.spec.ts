...
test('Cari Bagian Terisi', async ({ page }) => {
  ...
  await page.getByRole('combobox').selectOption('47');
  await page.goto('http://localhost:8000/DiklatInternal/detailperiod/list/44?periode_id=47');
  ...
  await page.getByRole('button', { name: 'Simpan' }).click();
    await expect(page.locator('text=Bagian berhasil ditambahkan!')).toBeVisible();
});