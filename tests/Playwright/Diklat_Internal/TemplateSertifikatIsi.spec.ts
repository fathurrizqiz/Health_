...
test('test', async ({ page }) => {
    ...
    await page.goto(
        'http://localhost:8000/RencanaDiklat/Internal/detail/aksi/44',
    );
    await page.getByRole('combobox').selectOption('47');
    ...
    await page
        .getByRole('textbox', { name: 'Nama materi / pembahasan' })
        .nth(1)
        .fill('Struktur Organisasi');

    await page.getByRole('button', { name: 'Simpan' }).click();
    await expect(page.locator('text=Template berhasil dibuat!')).toBeVisible();
});
